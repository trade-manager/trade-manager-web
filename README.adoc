...
6/ Install the DB. Change the following properties in the db/db.properties file and main pom.xml db-dba.user/db-dba.password/db-user.user/db-user.password set these to a user who can create schemas and users (root will do). From project dir, cd web, then run
