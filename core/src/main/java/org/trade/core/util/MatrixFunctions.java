...
public final class MatrixFunctions {
    ...
    public static synchronized boolean updateXYPairs(Hashtable<Long, Pair> userDataTable, double[] terms) {
        ...
        for (Iterator<Pair> enumPairs = userDataTable.values().iterator(); enumPairs.hasNext(); ) {
    ...
    public static synchronized double[] getCalculatedCoeffients(Pair[] data, int p) {
        ...
        // By request: read each datum only once not the most efficient processing method but required if the data set is huge
        ...
        // parent.show_mat(m); reduce matrix
        gj_echelonize(m);
        // parent.show_mat(m); extract rh column
