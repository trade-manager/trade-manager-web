...
The last bit of modeling security is to associate employees with a user. In this domain, an employee can have one user while a user can have multiple employees:
...
* The user attribute is linked via JPA's `@ManyToOne`. Manager doesn't need the `@OneToMany` because you haven't defined the need to look that up.
* The utility constructor call is updated to support initialization.

== Securing employees to their managers

Spring Security supports a multitude of options when it comes to defining security policies. In this section, you want to restrict things such that ONLY managers can view employee payroll data, and that saving, updating, and deleting operations are confined to the employee's user. In other words, any user can log in and view the data, but only a given employee's user can make any changes.
...
On `save()`, either the employee's user is null (initial creation of a new employee when no user has been assigned), or the employee's user's name matches the currently authenticated user's name. Here you are using http://docs.spring.io/spring-security/site/docs/current/reference/htmlsingle/#el-access[Spring Security's SpEL expressions] to define access. It comes with a handy "?." property navigator to handle null checks. It's also important to note using the `@Param(...)` on the arguments to link HTTP operations with the methods.

On `delete()`, the method either has access to the employee, or in the event it only has an id, then it must find the *employeeRepository* in the application context, perform a `findOne(id)`, and then check the user against the currently authenticated user.
...
A good user experience is when the application can automatically apply context. In this example, if a logged in user creates a new employee record, it makes sense for that user to own it. With Spring Data REST's event handlers, there is no need for the user to explicitly link it. It also ensures the user doesn't accidentally records to the wrong user.
...
In this sitation, you lookup the current user's security context to get the user's name. Then look up the associated user using `findByName()` and apply it to the user. There is a little extra glue code to create a new user if he or she doesn't exist in the system yet. But that is mostly to support initialization of the database. In a real production system, that code should be removed and instead depend on the DBAs or Security Ops team to properly maintain the user data store.
...
This merely adds a column for `this.props.employee.entity.user.name`.
...
The user field isn't something you want people to edit directly. Since it's inlined, it should be viewed as a read only attribute. To filter it out inlined entries from the `CreateDialog` and `UpdateDialog`, just delete such entries after fetching the JSON Schema metadata in `loadFromServer()`.
...
You can see the newly added user column. Go through a couple pages until you find employees owned by *oliver*.
...
* You defined the model of user and linked it to an employee via a 1-to-many relationship.
...
* You wrote another Spring Data REST event handler to trap creation events before they happen so they current user could be assigned as the employee's user.
* You updated the UI to show an employee's user and also display error pop-ups when unauthorized actions are taken.
