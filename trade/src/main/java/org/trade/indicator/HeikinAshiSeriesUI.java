...
public class HeikinAshiSeriesUI extends IndicatorSeriesUI {
    ...
    public HeikinAshiSeriesUI(Strategy strategy, String name, String type, String description, Boolean displayOnChart,
                              Integer chartRGBColor, Boolean subChart) {
        super(strategy, name, type, description, displayOnChart, chartRGBColor, subChart);

    }

    public HeikinAshiSeriesUI() {
        super(HeikinAshiSeries);
    }
    ...
    public RegularTimePeriod getPeriod(int index) {
        final HeikinAshiItemUI item = (HeikinAshiItemUI) getDataItem(index);
    ...
    public void add(Contract contract, RegularTimePeriod period, double open, double high, double low, double close,
        ...
        if (!this.isEmpty()) {
            HeikinAshiItemUI item0 = (HeikinAshiItemUI) this.getDataItem(0);
            if (!period.getClass().equals(item0.getPeriod().getClass())) {
                throw new IllegalArgumentException("Can't mix RegularTimePeriod class types.");
            }
        }
        super.add(new HeikinAshiItemUI(contract, period, open, high, low, close, lastUpdateDate), true);
    }
     ...
     */
    public void add(HeikinAshiItemUI dataItem, boolean notify) {
        if (!this.isEmpty()) {
            HeikinAshiItemUI item0 = (HeikinAshiItemUI) this.getDataItem(0);
            ...
            if (source.getItemCount() > 1) {
                ...
                if (index < 1) {
                    if (this.isEmpty()) {
                        ...
                        HeikinAshiItemUI prevItem = (HeikinAshiItemUI) this.getDataItem(this.getItemCount() - 1);
                        xClosePrev = prevItem.getClose();
                        xOpenPrev = prevItem.getOpen();
                    }
                } else {
                    HeikinAshiItemUI prevItem = (HeikinAshiItemUI) this.getDataItem(this.getItemCount() - 2);
                    xClosePrev = prevItem.getClose();
                    xOpenPrev = prevItem.getOpen();
                }
                ...
                if (index < 0) {
                    this.add(new HeikinAshiItemUI(source.getContract(), candleItem.getPeriod(), xOpen, xHigh, xLow,
                            xClose, candleItem.getLastUpdateDate()), false);
                } else {
                    HeikinAshiItemUI currDataItem = (HeikinAshiItemUI) this.getDataItem(index);
    ...
    public void printSeries() {
        for (int i = 0; i < this.getItemCount(); i++) {
            HeikinAshiItemUI dataItem = (HeikinAshiItemUI) this.getDataItem(i);
