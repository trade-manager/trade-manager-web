...
import org.trade.indicator.pivot.PivotItemUI;
...
public class PivotSeriesUI extends IndicatorSeriesUI {
     ...
     */
    public PivotSeriesUI(Strategy strategy, String name, String type, String description, Boolean displayOnChart,
    ...
    public RegularTimePeriod getPeriod(int index) {
        final PivotItemUI item = (PivotItemUI) getDataItem(index);
    ...
    public void add(RegularTimePeriod period, BigDecimal pivotPrice, String pivotSide) {
        if (!this.isEmpty()) {
            PivotItemUI item0 = (PivotItemUI) this.getDataItem(0);
            if (!period.getClass().equals(item0.getPeriod().getClass())) {
                throw new IllegalArgumentException("Can't mix RegularTimePeriod class types.");
            }
        }
        super.add(new PivotItemUI(period, pivotPrice, pivotSide), true);
    }
     ...
     */
    public void add(PivotItemUI dataItem, boolean notify) {
        if (!this.isEmpty()) {
            PivotItemUI item0 = (PivotItemUI) this.getDataItem(0);
     ...
     */
    public void createSeries(CandleDatasetUI candleDataset, int seriesIndex) {
    ...
    public void updateSeries(CandleSeriesUI source, int skip, boolean newBar) {
        ...
        CandleItemUI pivotCandle = null;
        CandleItemUI pivotRangeCandle = null;
        /*
        ...
        if ((startBar + 1) >= this.getBars()) {
            ...
            CandleItemUI prevCandle = null;
            for (int i = startBar; i > (startBar - this.getBars()); i--) {
                CandleItemUI candle = (CandleItemUI) source.getDataItem(i);
            ...
            if (pivot && (null != pivotCandle)) {
                ...
                try {
                    ...
                    if (null != entryLimit && entryLimit.getPivotRange().doubleValue() <= pivotRange.doubleValue()) {
                        ...
                        dataItem = new PivotItemUI(pivotCandle.getPeriod(), pivotPrice.getBigDecimalValue(), pivotSide);
                    }
                } catch (Exception ex) {
        ...
        if (null != dataItem) {
            if (this.indexOf(dataItem.getPeriod()) < 0) {
                ...
                PivotItemUI currDataItem = (PivotItemUI) this.getDataItem(this.indexOf(dataItem.getPeriod()));
    ...
    public void printSeries() {
        for (int i = 0; i < this.getItemCount(); i++) {
            PivotItemUI dataItem = (PivotItemUI) this.getDataItem(i);
