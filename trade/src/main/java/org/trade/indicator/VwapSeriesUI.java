...
import org.trade.indicator.vwap.VwapItemUI;
...
public class VwapSeriesUI extends IndicatorSeriesUI {
     ...
     */
    public VwapSeriesUI(Strategy strategy, String name, String type, String description, Boolean displayOnChart,
                        Integer chartRGBColor, Boolean subChart) {
        super(strategy, name, type, description, displayOnChart, chartRGBColor, subChart);
    }

    public VwapSeriesUI() {
        super(IndicatorSeriesUI.VwapSeries);
    }
    ...
    public RegularTimePeriod getPeriod(int index) {
        final VwapItemUI item = (VwapItemUI) getDataItem(index);
    ...
    public void add(RegularTimePeriod period, BigDecimal vwapPrice) {
        if (!this.isEmpty()) {
            VwapItemUI item0 = (VwapItemUI) this.getDataItem(0);
            if (!period.getClass().equals(item0.getPeriod().getClass())) {
                throw new IllegalArgumentException("Can't mix RegularTimePeriod class types.");
            }
        }
        super.add(new VwapItemUI(period, vwapPrice), true);
    }
     ...
     */
    public void add(VwapItemUI dataItem, boolean notify) {
        if (!this.isEmpty()) {
            VwapItemUI item0 = (VwapItemUI) this.getDataItem(0);
    ...
    public void updateSeries(CandleSeriesUI source, int skip, boolean newBar) {
        ...
        if (source.getItemCount() > skip) {
             ...
             */
            if (newBar) {
                VwapItemUI dataItem = new VwapItemUI(source.getRollingCandle().getPeriod(),
                        BigDecimal.valueOf(source.getRollingCandle().getVwap()));
                this.add(dataItem, false);
            } else {
                VwapItemUI dataItem = (VwapItemUI) this.getDataItem(this.getItemCount() - 1);
    ...
    public void printSeries() {
        for (int i = 0; i < this.getItemCount(); i++) {
            VwapItemUI dataItem = (VwapItemUI) this.getDataItem(i);
